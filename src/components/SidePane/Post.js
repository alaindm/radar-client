  ...
  render() {
    ...
    const conditionalPostLocationHeader = () => {
      if(post.location) {
        return (
          <div 
            onClick={ () => this.props.history.push(`/location/${post.location.id}`) } 
            style={{
              'cursor': 'pointer',
              'position': 'relative'
            }}
    ...
    if(post) {
      return (
        <div className="scroll">
          ...
          <Card className="card">
            <CardHeader
              title={<div onClick={ () => this.props.history.push(`/user/${post.owner.username}`) } style={{'cursor': 'pointer'}}>{post.owner.full_name}</div>}
              subtitle={conditionalPostLocationHeader()}
              avatar={post.owner.profile_pic_url}             
            />          
            <CardMedia
              overlay={<CardTitle title={moment(post.taken_at_timestamp * 1000).fromNow()} subtitle={`${post.edge_media_preview_like.count} likes`} />}
            >
              <img src={post.display_url} alt="from post" />
            </CardMedia>
            <CardTitle 
              subtitle={
                post.edge_media_to_caption.edges.map(x => {
                  return (
                    <div key={x.node.text}>
                      {x.node.text}
                    </div>
