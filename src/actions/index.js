...
export function logFBplaces() {
    return (dispatch, getState) => {
        ...
        .then(() => {
            ...
            let radius = 1000 * distance(centerLat, centerLng, cornerLat, cornerLng, "K")
...
export const getPlaces = (lat, lng, radius) => {
    let accessToken
    return axios.post(`${HOST}/places`, {accessToken , lat, lng, radius})
...
export const getSingleLocationPics = location => {
    ...
    return (dispatch, getState) => {
            dispatch({type: SET_LOADING})
            axios.post(`${HOST}/single-location-pics`, location)    
            .then(response => {
                dispatch({
    ...
    return (dispatch, getState) => {
        manageFbPermission(dispatch, getState)
            .then(() => {
                ...
                return axios.post(`${HOST}/multi-location-pics`, {accessToken , centerLat, centerLng, radius})
            })     
            .then(response => {
                dispatch({
                    type: GET_MULTI_LOCATION_PICS,
...
export const getUserInfo = username => {
    return dispatch => {
        dispatch({type: SET_LOADING})
        axios.post(`${HOST}/user-info`, {username})
...
export const getPostInfo = photoId => {
    return dispatch => {
        dispatch({type: SET_LOADING})
        axios.post(`${HOST}/post-info`, {photoId})
        .then(response => dispatch({
            type: GET_POST_INFO,
            data: response.data
        }))
...
function setAccessTokenState(dispatch, response = null) {
    if (response) {
        let expirationTimeCalc = Date.now() + (response.authResponse.expiresIn * 1000)
        dispatch({
...
export function zoomToArea(address) {
    ...
    return dispatch => {
        ...
        //   })
            ...
            //   if (status == google.maps.GeocoderStatus.OK) {
                dispatch({
                    type: SET_CENTER,
                    payload: [results[0].geometry.location.lat(), results[0].geometry.location.lng()]
                })
                // map.setZoom(15);
                // call the api
              } else {
